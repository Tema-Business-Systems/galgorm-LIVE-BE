...
public class AsyncSchdulerService {
    ...
    private String DOCS_QUERY = "select d.*, x.PRODUCTCODE, x.PRODUCTNAME,x.PRODUCTCATEG, x.QUANTITY, x.UOM, x.DOCLINENO, x.CONV_QTY, x.PURUNIT, x.PRODUCTWEIGHT, x.PRODUCTVOLUME, x.PRODUCTWUNIT, PRODUCTVUNIT \n" +
    ...
    private String DOCS_QUERY2 = "select d.*, x.PRODUCTCODE, x.PRODUCTNAME,x.PRODUCTCATEG, x.QUANTITY, x.UOM, x.DOCLINENO, x.CONV_QTY, x.PURUNIT, x.PRODUCTWEIGHT, x.PRODUCTVOLUME, x.PRODUCTWUNIT, PRODUCTVUNIT \n" +
    ...
    private VehicleVO convert(Vehicle vehicle, List<String> vehicleSList) {
        ...
        vehicleVO.setMondayRC(vehicle.getMondayRC());
        vehicleVO.setMondayRCDesc(vehicle.getMondayRCDesc());
        vehicleVO.setTuesdayRC(vehicle.getTuesdayRC());
        vehicleVO.setTuesdayRCDesc(vehicle.getTuesdayRCDesc());
        vehicleVO.setWednesdayRC(vehicle.getWednesdayRC());
        vehicleVO.setWednesdayRCDesc(vehicle.getWednesdayRCDesc());
        vehicleVO.setThursdayRC(vehicle.getThursdayRC());
        vehicleVO.setThursdayRCDesc(vehicle.getThursdayRCDesc());
        vehicleVO.setFridayRC(vehicle.getFridayRC());
        vehicleVO.setFridayRCDesc(vehicle.getFridayRCDesc());
        vehicleVO.setSatdayRC(vehicle.getSatdayRC());
        vehicleVO.setSatdayRCDesc(vehicle.getSatdayRCDesc());
        vehicleVO.setSundayRC(vehicle.getSundayRC());
        vehicleVO.setSundayRCDesc(vehicle.getSundayRCDesc());
...
//        }
    ...
    private String getGroupingColor(Date docdate) {
        ...
        switch (day) {
            ...
            case 6 :
                ...
                break;
            case 7 :
                groupColor = ";font-style:normal;background-color:#4831D4";
                break;
        }
        return groupColor;
    }
  ...
  productVO.setUom(this.convertToString(map.getUom()));
    ...
    private ProductVO getProductVO(Map<String, Object> map) {
        ...
        productVO.setPweight(this.convertToString(map.get("PRODUCTWEIGHT")));
        productVO.setPvolume(this.convertToString(map.get("PRODUCTVOLUME")));
        productVO.setPwunit(this.convertToString(map.get("PRODUCTWUNIT")));
        productVO.setPvunit(this.convertToString(map.get("PRODUCTVUNIT")));
...
//        }
    ...
    public List<DocsVO> getDocsWithRange2(List<String> site, Date sdate, Date edate) {
        ...
        if(!StringUtils.isEmpty(site)) {
           ...
           resultList2 = jdbcTemplate.queryForList(MessageFormat.format(DOCS_QUERY2, dbSchema,
                    MessageFormat.format(SITE_DATERANGE_OPENRANGE, Sites, dateFormat.format(sdate),dateFormat.format(edate))), paramMap);
