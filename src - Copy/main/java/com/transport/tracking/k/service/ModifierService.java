...
public class ModifierService {
    ...
    private List<OpenToAddDocsVO> convertDocs3(List<Map<String, Object>> list) {
        Map<String, OpenToAddDocsVO> dropsMap = new HashMap<>();
        for(Map<String, Object> map: list) {
            String docNum = this.convertToString(map.get("DOCNUM"));
            OpenToAddDocsVO dropsVO = null;
            if(Objects.nonNull(dropsMap.get(docNum))) {
                dropsVO = dropsMap.get(docNum);
            }else {
                dropsVO = new OpenToAddDocsVO();
            }
            dropsVO.setDocnum(docNum);

            this.convertDocs3(map, dropsVO);
    ...
    private OpenToAddDocsVO convertDocs3(Map<String, Object> drops, OpenToAddDocsVO dropsVO) {
        ...
        dropsVO.setVolume(this.convertToString(drops.get("VOLUME")));
        dropsVO.setVunit(this.convertToString(drops.get("VOLUME_UNIT")));
        dropsVO.setWeight(this.convertToString(drops.get("NETWEIGHT")));
        dropsVO.setWunit(this.convertToString(drops.get("WEIGHTUNIT")));
        if(Objects.isNull(dropsVO.getProducts())) {
            List<ProductVO> productVOS = new ArrayList<>();
            dropsVO.setProducts(productVOS);
        }
    ...
    public List<OpenToAddDocsVO> getOpenToAddDocsWithRange(List<String> site, Date sdate, Date edate) {
        List<OpenToAddDocs> drops = null;
        List<OpenToAddDocsVO> dropsList = new ArrayList<>();
        Map<String, String> paramMap = new HashMap<>();
        List<Map<String, Object>> resultList2 = new ArrayList<>();
        List<OpenToAddDocs> resultList = new ArrayList<>();
        String Sites =  this.ListtoString(site);
        log.info("Sch - inside Range2");
        log.info(site.toString());
        if(!StringUtils.isEmpty(site)) {
            ...
            resultList2 = jdbcTemplate.queryForList(MessageFormat.format(OPENTOADDDOCS_QUERY, dbSchema,
                    MessageFormat.format(SITE_DATERANGE, Sites, dateFormat.format(sdate),dateFormat.format(edate))), paramMap);
        ...
        if(!CollectionUtils.isEmpty(resultList2)) {
            /*dropsList = resultList.stream().map(a-> this.convertDrops(a))
                    .collect(Collectors.toList());*/
            dropsList = this.convertDocs3(resultList2);
            // List<DocsVO> finallist = sortedDocs(dropsList);
            return dropsList;
            // return resultList2;
        }
            ...
            List<OpenToAddDocsVO> emptyDocs = new ArrayList<>();
            dropsList = emptyDocs;
        }
        return dropsList;
    }
